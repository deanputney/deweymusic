...
{
$query = "SELECT artists.id, artists.name FROM artists JOIN artists_genres ON artists_genres.artist_id = artists.id WHERE artists_genres.genre_id = $genre_id ORDER BY artists.name";
$result = mysql_query($query);
...
function getArtistByDeanId($deanId)
{

$query = "SELECT id FROM artists WHERE dean_id = $deanId";
$result = mysql_query($query);
